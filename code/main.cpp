...
bool RemoveVertexFromSet(Vertex* v) {
	...
	if (v == old_Set->firstVertex && v == old_Set->lastVertex) {
		if (old_Set != initSet) {
			if (old_Set == firstSet) firstSet = firstSet->nextSet;
			delete old_Set;
			isSetRemoved = true;
		}
		else {
		...
		if (v == old_Set->firstVertex) old_Set->firstVertex = v->next;
		if (v == old_Set->lastVertex) old_Set->lastVertex = v->prev;
	}
...
void AddVertexToSet(Vertex* v) {
	...
	if (new_Set->nextSet) {
		...
		if (new_Set->nextSet->firstVertex) new_Set->nextSet->firstVertex->prev = v;
	}
    // else v->next = NULL
...
void RelocateVertex(Vertex* v, StackOfSet* listOfDividedSets) {
	...
	if (!old_Set->isDivided) {
		...
		if (old_Set == firstSet) firstSet = old_Set->newSet;
	}
...
void LexBFS(ArrayOfVertex* tableOfVertices, ArrayOfVertex2D* neighbourhood, Mode mode) {
	...
	while (firstVertex)
	{
		...
		for (i = 0; i < rank; i++) {
			...
			if (neighbour->state == NOT_VISITED || neighbour->state == VISITED) RelocateVertex(neighbour, listOfDividedSets);
		}
...
void main() {
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 0; j < k; j++) {
			scanf("%d", &t);
			if (t == 1) LexBFS(tableOfVertices, neighbourhood, BASIC);
			else LexBFS(tableOfVertices, neighbourhood, PLUS);
		}
